...
import { PlatformWebSearchCommand } from "./cli/cmd/platform-web-search"
...
import { PlatformAtlasStorageCommand } from "./cli/cmd/platform-atlas-storage"
import { PlatformMintCommand } from "./cli/cmd/platform-mint"
import { PlatformOkrCommand } from "./cli/cmd/platform-okr"
...
import { PlatformAtlasDatabaseCommand } from "./cli/cmd/platform-atlas-database"
...
import { PlatformCommerceCommand } from "./cli/cmd/platform-commerce"
...
const cli = yargs(rawArgs)
  ...
  .command(reg(PlatformWebSearchCommand))
  ...
  .command(reg(PlatformAtlasStorageCommand))
  ...
  .command(reg(PlatformAtlasDatabaseCommand))
  ...
  // is a product noun ("my files"), not a transport detail, and because an always-on or
  // API-backed replica would make the mesh optional — at which point nesting it under `hive`
  // would imply a requirement that no longer exists.
  ...
  .command(reg(PlatformCommerceCommand))

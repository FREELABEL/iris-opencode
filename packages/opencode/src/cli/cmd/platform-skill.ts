...
const SkillRunCommand = cmd({
  ...
  async handler(args) {
    await withInstance(async () => {
      ...
      console.log()
      printDivider()
      const statusIcon = result.status === "completed" ? success("✓") : "✗"
      console.log(`  ${statusIcon} ${bold(result.skill)} — ${result.status}`)
      console.log(dim(`  Run ID: ${result.run_id}`))

      const totalMs = Object.values(result.steps).reduce((sum, r) => sum + r.duration_ms, 0)
      console.log(dim(`  Duration: ${(totalMs / 1000).toFixed(1)}s`))
      printDivider()
...
function modeLabel(mode: string): string {
  switch (mode) {
    ...
    case "skill": return highlight("skill")
...
// ============================================================================
// Parent command: iris skill <subcommand>
// ============================================================================

export const PlatformSkillCommand = cmd({
  command: "skill <subcommand>",
  describe: "unified skill system — local v2 execution + remote agent skills + review queue",
  builder: (yargs) =>
    yargs

...
async function walk(
  ...
  for (const f of files) {
    ...
    console.log(`${prefix}${f.name}  ${dim(kind)}`)
  }
  ...
  for (const d of folders) {
    ...
    if (depth + 1 < maxDepth) {
      await walk(d.id, driveId, depth + 1, maxDepth, pageSize, prefix + "  ", counts)
    } else {
...
export const PlatformDriveCommand = cmd({
  ...
  builder: (y) =>
    y
      .positional("action", {
        describe: "list-drives | tree",
        type: "string",
        choices: ["list-drives", "tree"],
      })
  ...
  async handler(args) {
    ...
    try {
      ...
      await walk(folderId, driveId, 0, maxDepth, pageSize, "  ", counts)

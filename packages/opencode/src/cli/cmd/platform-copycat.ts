...
// Transcribe is an alias — `iris copycat transcribe` forwards to `iris transcribe`.
// Single implementation, no drift.
import { PlatformTranscribeCommand as _TranscribeCore } from "./transcribe"
const TranscribeCommand = cmd({
  command: "transcribe <url>",
  describe: "transcribe a video — alias for `iris transcribe`",
  builder: _TranscribeCore.builder,
  handler: _TranscribeCore.handler,
})

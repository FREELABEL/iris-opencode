...
export const ReachrScrapeCmd = cmd({
  ...
  describe: "find people on public pages (team, about, directories) — dry run; --write adds them as leads",
  builder: (y: any) =>
    y
  ...
  async handler(args: any) {
    ...
    if (!args.write) {
      ...
      for (const l of leads) {
        const marks = [l.email && "email", l.phone && "phone", Object.keys(l.socials ?? {}).length && "social"]
          .filter(Boolean)
          .join(" ")
        out(`  ${bold(l.name.v)}${l.title ? dim(` — ${l.title.v}`) : ""}`)
        out(dim(`      ${l.company ?? ""}  ·  ${marks || "no contact"}  ·  evidence: ${(l.evidence ?? ["json-ld"]).join(", ")}`))
      }
    ...
    for (const l of toWrite) {
      ...
      try {
        ...
        const payload: Record<string, unknown> = { name, bloqId, source: "reachr-scrape" }

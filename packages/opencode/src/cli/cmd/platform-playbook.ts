 ...
 * In the HOME skills dir (~/.claude/skills) two things are left alone rather than overwritten:
 * a SKILL.md that `sync` did not generate (a hand-written skill), and a symlinked skill folder —
 * on this author's machine every ~/.claude/skills entry links into a repo checkout, and writing
 * through it would rewrite that repo's files.
 */
function writeSkillReplica(skillsDir: string, name: string, output: string, opts: { home: boolean }): { ok: true } | { ok: false; reason: string } {
  const targetDir = pathJoin(skillsDir, name)
  const targetFile = pathJoin(targetDir, "SKILL.md")
  if (opts.home) {
    if (isSymlink(targetDir)) return { ok: false, reason: "a symlinked folder — left alone" }
    if (existsSync(targetFile)) {
      let existing = ""
      try { existing = readFileSync(targetFile, "utf8") } catch { /* unreadable → treat as not ours */ }
      if (!existing.includes("AUTO-GENERATED by iris playbook sync")) {
        return { ok: false, reason: "a hand-written skill of the same name — left alone" }
      }
    }
  }
  writeFileAtomic(targetFile, output)
  return { ok: true }
}
  ...
  async handler(args) {
    ...
    if (syncSkills) {
      try {
        ...
        const plan = await parsePlan(info)
        const output = await renderSkillReplica(info, plan)
        const r = writeSkillReplica(root.skillsDir, plan.name, output, { home: root.scope === "global" })
        if (r.ok) skillFile = pathJoin(root.skillsDir, plan.name, "SKILL.md")
    ...
    if (skillNote) console.log(dim(`  Skill not refreshed: ${skillNote}`))
